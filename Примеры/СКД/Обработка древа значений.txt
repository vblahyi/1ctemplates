...
Процедура ОтметитьВсеСтроки(Пометка) 
   Для Каждого ТекущаяСтрока Из ТаблицаРезультат.Строки Цикл 
       ТекущаяСтрока["Пометка"] = ?(Пометка=Неопределено,ПолучитьОбратнуюОтметку(ТекущаяСтрока["Пометка"]),Пометка); 
       ОтметитьСтрокиНижеТекущей(ТекущаяСтрока,Пометка); 
   КонецЦикла; 
КонецПроцедуры 
